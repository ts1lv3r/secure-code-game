...
class TestOnlineStore(unittest.TestCase):
    ...
    def test_6(self):
        """
        My Thoughts:
            item.amountにおいてfloatを使っているがためにtvの料金1000ドルがpaymentとpaybackの大きな値によって近似されてしまっている
        Solutions:
            decimal.Decimalでamountを囲む
        """
    ...
    def test_7(self):
        """
        My Thoughts:
            浮動小数点型の演算のため小さい数字の減算がおかしくなる
        Solutions:
            if演算時にnetをround()で囲む？
            => Decimal()時に中でstr()変換する。つまり`Decimal(str(?))`の形でデータの保持と演算を行う。
        """
